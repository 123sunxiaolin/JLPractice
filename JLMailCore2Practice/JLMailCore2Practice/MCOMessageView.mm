...
- (id)initWithFrame:(CGRect)frame
{
    ...
    for (UIView *subView in _webView.subviews) {
        if ([subView isKindOfClass:[UIScrollView class]]) {
            subView.frame = CGRectMake(0, 100, CGRectGetWidth(_webView.frame), CGRectGetHeight(_webView.frame));
        }
    }
    //_webView.scrollView.frame = CGRectMake(0, 100, CGRectGetWidth(_webView.frame), CGRectGetHeight(_webView.frame));
...
- (void) _refresh
{
    ...
    if (_message == nil) {
        ...
        if ([_message isKindOfClass:[MCOIMAPMessage class]]) {
            NSArray *array = [(MCOIMAPMessage *) _message requiredPartsForRendering];
    ...
    [html appendFormat:@"<html><head><script>%@</script><style>%@</style></head>"
        @"<body><div contenteditable=\"true\">%@ </div></body><iframe src='x-mailcore-msgviewloaded:' style='width: 0px; height: 0px; border: none;'>"
...
- (void)webViewDidFinishLoad:(UIWebView *)webView{
    NSString *string = [_webView stringByEvaluatingJavaScriptFromString:@"document"];
    
    for (UIView *subView in _webView.subviews) {
        if ([subView isKindOfClass:[UIScrollView class]]) {
            subView.frame = CGRectMake(0, 200, CGRectGetWidth(_webView.frame), CGRectGetHeight(_webView.frame));
        }
    }
